...
class Candidate extends CI_Controller
{
	...
	{
		$categoryID = $this->UserModel->getUserByUserID($_GET['id']);
		$work_type = $this->UserModel->getCandidateCategory($categoryID[0]->user_job);

		$data['user_id'] = $_GET['id'];
		$data['type'] = $work_type[0]->job_name;
		$data['nav'] = '';
	...
	function getProfile(){
		...
		$result = $this->ProfileModel->getCandidateProfile($this->input->post('id'));
	...
	public function addReview(){
		...
		if($result){
			...
			$reviewUpdate = $this->ReviewModel->getCandidateReview($this->input->post('txtUserId'), $this->input->post('txtProfileId'));
			$this->ProfileModel->updateRatingValueCandidate($reviewUpdate['review_rating'], $this->input->post('txtUserId'), $this->input->post('txtProfileId'));
	...
	function getReviews(){
		...
		$profileId = $this->ProfileModel->getCandidateProfileID($this->input->post('user_id'));

		$result = $this->ReviewModel->getAllReviews($this->input->post('user_id'), $profileId[0]->candidate_profile_id);
