...
<div class="col-lg-8 col-xl-9">
	...
	<div class="row applyed_job">
		<div class="table-responsive job_review_table">
			<table id="tblAppliedJobs" class="table">
			</table>
		</div>
	</div>
</div>
...
<script>
		...
		$.confirm({
			...
			buttons: {
				confirm: {
					...
					action: function(){
						$.ajax({
							url: "<?php echo base_url(''); ?>/BCAppliedJobs/deleteRequestRecord",
							data: {ID : id},
							method: "post",
							dataType: "json",
	...
	function view(id){
		$.ajax({
			url: "<?php echo base_url(''); ?>/BCAppliedJobs/updateReadStatus",
			data: {
				ID: id
			},
			method: "post",
			...
			success: function(r){
				location.href = '<?php echo base_url('Job'); ?>?job_id=' + r.data[0].job_id+'&user_id='+r.data[0].company_user_id;
	...
	function initTable(){
		dTable = $('#tblAppliedJobs').DataTable({
			"processing": true,
			...
			"columns": [
				...
				{"data": "company_name", "name": "Company Name", "title": "Company Name"},
				{"data": "company_phone", "name": "Company Contact No.", "title": "Company Contact No."},
			...
			"ajax": {
				"url": '<?php echo base_url(''); ?>/BCAppliedJobs/getAllAppliedJobs',
				"dataType": "json",
			},
			"oLanguage": {
